...
`cobalt-ops-cleanup-deck` is centered on automation work in JavaScript. The technical target is: Develop a JavaScript command-oriented project for cleanup scenarios with negative fixtures, human-readable error snapshots, and local-only command execution.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 2.
